...
SupportPilot 是一个可运行的 AI 客户支持作品集项目。客户获得经过内部知识库校验的回答；证据不足时系统最多澄清一次，再生成可编辑工单草稿，并仅在客户确认后转交人工支持。内部引用、候选片段和检索分数只对支持人员的 RAG 验收页面开放。
...
演示环境使用预置账号。真实 B2B SaaS 形态采用企业管理员邀请制，不开放公共注册；公开注册、密码找回、企业管理和计费流程不在本作品集范围内。
...
2. 查看客户侧最终回答，确认页面不暴露内部文档、引用和检索分数。
3. 提出实例问题，或说“请转人工并创建工单，我的生产工作流持续失败”。
4. 编辑 Agent 生成的工单预览并确认。
5. 用人工支持账号进入“RAG 验收”，查看候选片段、最终引用和工具耗时。
6. 认领、回复并标记解决。
