...
async function claim() {
  ...
  actionError.value = ''
  try {
    active.value = await api<Ticket>(`/tickets/${active.value.id}/claim`, { method: 'POST' })
    await load()
  } catch (cause) {
    actionError.value = cause instanceof Error ? cause.message : '认领失败，请重试'
  } finally {
    ...
    <section class="support-workspace" :class="{ 'detail-open': active }">
      ...
      <article v-if="active" class="support-detail">
        ...
        <div class="support-detail-actions">
          <button v-if="!active.assignee_id" class="primary-button" :disabled="actionLoading" @click="claim"><LoaderCircle v-if="pendingAction === 'CLAIM'" class="spin" :size="16" /><UserCheck v-else :size="16" />认领并开始处理</button>
          <template v-else><button v-if="active.status === 'IN_PROGRESS'" class="secondary-button" :disabled="actionLoading" @click="update({ status: 'WAITING_CUSTOMER' })">等待客户补充</button><button v-if="!['RESOLVED','CLOSED'].includes(active.status)" class="primary-button" :disabled="actionLoading" @click="update({ status: 'RESOLVED' })"><LoaderCircle v-if="pendingAction === 'RESOLVED'" class="spin" :size="16" /><CheckCircle2 v-else :size="16" />标记已解决</button></template>
          <label class="inline-select">分类<select :value="active.category" :disabled="actionLoading" @change="update({ category: ($event.target as HTMLSelectElement).value })"><option v-for="category in ['ACCOUNT','CONFIG','API','BILLING','INCIDENT','FEATURE','OTHER']" :key="category" :value="category">{{ categoryLabel(category) }}</option></select></label>
          <label class="inline-select">优先级<select :value="active.priority" :disabled="actionLoading" @change="update({ priority: ($event.target as HTMLSelectElement).value })"><option value="LOW">低</option><option value="NORMAL">普通</option><option value="HIGH">高</option></select></label>
        </div>
        <div class="support-detail-scroll">
          <p v-if="actionError" class="form-error support-action-error">{{ actionError }}</p>
        ...
        <footer class="support-reply"><textarea v-model="reply" rows="2" placeholder="回复客户，说明下一步或处理结果" :disabled="actionLoading"></textarea><button class="send-button" title="发送回复" :disabled="!reply.trim() || actionLoading" @click="addReply"><LoaderCircle v-if="pendingAction === 'REPLY'" class="spin" :size="18" /><Send v-else :size="18" /></button></footer>
      </article>
    </section>
  </div>
</template>
